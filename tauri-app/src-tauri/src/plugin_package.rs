...
impl PluginPackage {
    ...
    pub fn from_zip_bytes(data: &[u8]) -> Result<Self> {
        ...
        let manifest_file = archive
            ...
            .inspect_err(|e| {
                tracing::error!("插件包中未找到 manifest.json: {}", e);
            })
    ...
    pub fn install(&self, plugin_dir: &Path) -> Result<()> {
        ...
        let mut archive = ZipArchive::new(cursor).inspect_err(|e| {
            tracing::error!("重新打开 ZIP 归档失败: {}", e);
        })?;
    ...
    pub fn get_library_path(&self, plugin_dir: &Path) -> Result<PathBuf> {
        let lib_name = self
            ...
            .ok_or_else(|| {
                tracing::error!("未配置当前平台的动态库文件");
                anyhow::anyhow!("当前平台不受支持")
            })?;
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        let lib_name = match self.get_library_filename() {
            ...
            None => {
                ...
                anyhow::bail!("未配置当前平台的动态库文件");
            }
        };

        let cursor = Cursor::new(&self.archive_data);
        let archive = ZipArchive::new(cursor).inspect_err(|e| {
            tracing::error!(
                plugin_id = %self.manifest.id,
                "重新打开 ZIP 归档失败: {}",
                e

...
fn load_connections() -> Vec<ConnectionConfig> {
    ...
    let path = home.join(".worktools").join("history").join("plugins").join("object-storage.json");
    if !path.exists() {
    ...
    match arr {
        Some(list) => list.iter().map(|v| {
            serde_json::from_value::<ConnectionConfig>(v.clone()).unwrap()
        }).collect(),
        None => vec![],
    }
}
...
fn test_list_buckets_aliyun() {
    ...
    let conn = connections.iter().find(|c| c.provider == "aliyun")
...
fn test_list_buckets_tencent() {
    let connections = load_connections();
    let conn = connections.iter().find(|c| c.provider == "tencent")
        .expect("未找到腾讯云连接");

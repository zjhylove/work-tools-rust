...
/// 生成请求参数的 mock JSON
pub fn generate_req_mock_json(fields: &[ApiField]) -> String {
    if fields.is_empty() {
        return "{}".to_string();
    }
    ...
    for (i, field) in fields.iter().enumerate() {
        let value = mock_value_for_type(&field.field_type, &field.example_value);
...
fn generate_node_mock(node: &NodeInfo, all_nodes: &[NodeInfo], indent: &str) -> String {
    ...
    for (i, field) in node.resp_fields.iter().enumerate() {
        ...
        let child_node = all_nodes.iter().find(|n| short_name(&n.node_name) == field_short);
